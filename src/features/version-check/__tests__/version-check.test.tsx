...
import { CHECK_INTERVAL_MS, CHECK_TIMEOUT_MS, LATEST_RELEASE_API } from "@/features/version-check/constants";
...
const mockResponse = (data: unknown, status = 200) =>
  vi.fn().mockResolvedValue({ ok: status === 200, status, json: async () => data });
const stableRelease = (version = "v99.0.0") => ({ tag_name: version, draft: false, prerelease: false });
...
describe("version comparison", () => {
  ...
  it.each(["latest", "2.1", "02.1.0", "2.1.0-01", "2.1.0-", "2.1.0/evil"])("rejects invalid version %s", (version) => {
...
describe("version sources", () => {
  it("uses GitHub stable releases for desktop and constructs a trusted link", async () => {
    const fetch = mockResponse({ ...stableRelease(), html_url: "https://example.com/unsafe" });
    vi.stubGlobal("fetch", fetch);
    const signal = new AbortController().signal;
    expect(await fetchLatestVersion(true, signal)).toEqual({
      version: "99.0.0", updateAvailable: true,
      releaseUrl: "https://github.com/HitBox38/Unfurl/releases/tag/v99.0.0",
    });
    expect(fetch).toHaveBeenCalledWith(LATEST_RELEASE_API, expect.objectContaining({ signal, credentials: "omit" }));
  });

  it("checks the deployed web manifest without using the browser cache", async () => {
    const fetch = mockResponse({ version: "99.0.0" });
    vi.stubGlobal("fetch", fetch);
    expect(await fetchLatestVersion(false, new AbortController().signal)).toMatchObject({ updateAvailable: true });
    expect(fetch).toHaveBeenCalledWith(expect.stringMatching(/^\/version\.json\?t=\d+$/), expect.objectContaining({ cache: "no-store" }));
  });

  it("handles no published release", async () => {
    vi.stubGlobal("fetch", mockResponse({}, 404));
    expect(await fetchLatestVersion(true, new AbortController().signal)).toBeNull();
    await expect(fetchLatestVersion(false, new AbortController().signal)).rejects.toThrow();
  });

  it.each([{}, null, { ...stableRelease(), prerelease: true }, { ...stableRelease(), draft: true }, stableRelease("invalid")])("rejects malformed or unpublished desktop data: %j", async (data) => {
    vi.stubGlobal("fetch", mockResponse(data));
    await expect(fetchLatestVersion(true, new AbortController().signal)).rejects.toThrow();
  });

  it.each([403, 429, 500])("treats HTTP %s as a failed check", async (status) => {
...
describe("automatic checks", () => {
  it("checks once in StrictMode, throttles focus, polls, and cleans up", async () => {
    ...
    vi.stubGlobal("fetch", fetch);
    const { result, unmount } = renderHook(() => useVersionCheck(false), { wrapper: StrictMode });
  ...
  it("aborts a stalled check and allows retry without overlapping requests", async () => {
    vi.useFakeTimers();
    const fetch = vi.fn((_url: string, { signal }: RequestInit) => new Promise((_resolve, reject) => {
      signal?.addEventListener("abort", () => reject(new Error("aborted")));
    }));
    vi.stubGlobal("fetch", fetch);
    const { result, unmount } = renderHook(() => useVersionCheck(false));
    act(() => { void result.current.check(true); void result.current.check(true); });
    expect(fetch).toHaveBeenCalledTimes(1);
...
async function openUpdates(desktop = false) {
  if (desktop) Object.defineProperty(window, "ipcRenderer", { configurable: true, value: {} });
  render(<TooltipProvider><SidebarProvider><VersionCheckHost desktop={desktop} /></SidebarProvider></TooltipProvider>);
  await userEvent.click(screen.getByRole("button", { name: /Unfurl v.*Check for updates/ }));
...
describe("update dialog", () => {
  it("offers desktop downloads when a new release is available", async () => {
    ...
    expect(await screen.findByText("Unfurl v99.0.0 is available.")).toBeInTheDocument();
    expect(screen.getByRole("link", { name: "Download update" })).toHaveAttribute("href", "https://github.com/HitBox38/Unfurl/releases/tag/v99.0.0");
  ...
  it("reports up to date and lets a failed check be retried", async () => {
    const fetch = mockResponse({ version: __APP_VERSION__ }).mockRejectedValueOnce(new Error("offline"));
    vi.stubGlobal("fetch", fetch);
    await openUpdates();
