...
export function useVersionCheck(desktop: boolean, automatic = true) {
  const [result, setResult] = useState<VersionResult | null>(null);
  const [status, setStatus] = useState<
    "idle" | "checking" | "checked" | "error"
  >("idle");
    ...
    async (manual = false) => {
      if (
        ...
        (!manual &&
          ...
          Date.now() - lastCheck.current < CHECK_INTERVAL_MS)
      )
        return;
  ...
  useEffect(() => {
    if (!automatic) return;
    const autoCheck = () => {
      if (!import.meta.env.DEV && document.visibilityState === "visible")
        void check();
    };
